      ...
      {freshnessWarn && (
        <div style={{
          ...
          <div style={{display:"flex",alignItems:"center",gap:8}}>
            ...
            <div>
              <span style={{fontSize:12,fontWeight:700,color:"#ffd166"}}>Données potentiellement périmées</span>
              <span style={{fontSize:11,color:"#ffa94d",marginLeft:8}}>
                JSON mis à jour il y a {freshnessAge} min · GitHub Actions a peut-être raté un cycle
              </span>
            </div>
          </div>
          <button onClick={load} style={{
            background:"rgba(243,156,18,.2)",color:"#ffd166",
            border:"1px solid rgba(243,156,18,.4)",borderRadius:6,
            padding:"4px 12px",fontSize:11,cursor:"pointer",fontFamily:"monospace",whiteSpace:"nowrap"
          }}>⟳ FORCER RELOAD</button>
        </div>
      )}
      {/* Indicateur fraîcheur OK discret */}
      {!freshnessWarn && freshnessAge !== null && (
        <div style={{marginBottom:10,fontSize:10,color:"#2ecc71",display:"flex",alignItems:"center",gap:6}}>
          <span style={{width:6,height:6,borderRadius:"50%",background:"#2ecc71",display:"inline-block"}}/>
          Données fraîches · mises à jour il y a {freshnessAge} min
          {d.pipeline_status && d.pipeline_status !== "ok" &&
            <span style={{color:"#ffa94d",marginLeft:8}}>· Pipeline: {d.pipeline_status}</span>}
        </div>
      )}
