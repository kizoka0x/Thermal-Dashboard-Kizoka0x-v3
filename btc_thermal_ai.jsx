...
function BTCThermalAI() {
  ...
  // ── DIM 3 : PROFITABILITÉ — SOPR (w3) + LTH (w2) + STH (w2) + UTXO (w1) ─
  // SOPR Alert ~0.4-0.5 = capitulation historique | LTH<0 = configuration bottom
  const D_PROFIT = (()=>{
    const sopr=G("soprRatio",1), soprA=G("soprAlert");
    const lth=G("lthNupl"), sth=G("sthNupl"), utxo=G("utxoRatio");
    const sS=sopr>=3?10:sopr>=1.05?7:sopr>=0.995?4:sopr>=0.97?3:sopr>=0.70?2:sopr>=0.54?1:0;
    ...
    const uS=utxo>=75?9:utxo>=55?7:utxo>=35?5:utxo>=15?3:utxo>=5?2:1;
    ...
    return{score,regime,soprCapit:soprA===1,lthEnPerte:lth<0,sthEnPerte:sth<0,utxoFlag:G("utxoFlag")===1};
  })();
  ...
  const MARKET_LOGIC = (()=>{
    ...
    const {score:pr,soprCapit,lthEnPerte,sthEnPerte,utxoFlag:utxoFl}=D_PROFIT;
    ...
    // Checklist Bottom pondérée — 3 niveaux (rapport 26 fév)
    const checksBot=[
      ...
      {ok:G("soprAlert")===1,   label:"SOPR Alert (~0.4–0.5)",    cat:"HAUTE",   w:2,ctx:"Capitulation LTH. Signal achat fort historique."},
      ...
      {ok:G("sharpeShort")<-0.3,label:"Sharpe < −0.3 (Low Risk)", cat:"LT",      w:1,ctx:"4ème occurrence depuis 2012."},
    ...
    const checksTop=[
      ...
      {ok:G("soprRatio")>3.0,     label:"SOPR Ratio > 3.0 (bull line)",    cat:"HAUTE",   w:2},
      ...
      {ok:G("mayerMultiple")>2.0, label:"Mayer Multiple > 2.0",            cat:"LT",      w:1},
      {ok:G("sharpeShort")>1.0,   label:"Sharpe > 1.0",                    cat:"LT",      w:1},
    ...
    const zones=[
      ...
      {range:"$"+Math.round(sma200*0.55/1000)+"–"+Math.round(sma200*0.70/1000)+"K",
       ...
       label:soprCapit&&utxoFl?"⚡ SOPR Alert + UTXO Flag actifs!":"⚡ Si SOPR Alert=1 + UTXO Flag=1",
