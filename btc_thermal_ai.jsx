...
//  - UTXO Flag = 1 quand ratio P/L réel < 3 (proxy : utxoRatio × 711 < 3)
// ══════════════════════════════════════════════════════════════════════════════
...
function BTCThermalAI() {
  ...
  const checksBot = [
    ...
    { ok: G("utxoFlag")       === 1,        label: "UTXO Flag = 1 (ratio P/L < 3)"      },
  ...
  const rows = [
    ...
    },
    {
      ...
      sub_detail:<>
        <SubVal label="Ratio" val={f2(G("utxoSma365"))} color={G("utxoSma365")<3?"#ff4444":G("utxoSma365")<6?"#ffa94d":"#c9d1d9"}
          alert={<AlertR active={G("utxoFlag")===1} on="FLAG=1 🚨"/>}/>
        <SubVal label="SMA-365d [ratio×711]" val={f2(G("utxoSma365"))} color={G("utxoSma365")<6?"#ffa94d":"#74c0fc"}/>
        <SubVal label="Flag" val={G("utxoFlag")===1?"= 1 ⚡":"= 0"} color={G("utxoFlag")===1?"#ff4444":"#69db7c"}/>
      </>,
      sc:s.utxo, nowL:G("utxoFlag")===1?"FLAG\nACTIF":G("utxoSma365")<6?"BAS\nCYCLE":"MILIEU",hz:"MT/LT"
    },
    {
      sec:null,
