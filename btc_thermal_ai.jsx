...
//  - UTXO Flag = 1 quand ratio ≤ 3 (valeur CQ proxy : ≤ 0.03 de notre calc)
// ══════════════════════════════════════════════════════════════════════════════
...
// UTXO ratio 0-1 (proxy valeur CQ — fond cycle ~3 → notre 0.03)
const S_UTXO = v =>
  v<=0.02?{n:1,c:3,m:5,l:9}:v<=0.10?{n:2,c:3,m:5,l:8}:v<=0.25?{n:3,c:4,m:5,l:7}:
  v<=0.50?{n:5,c:5,m:5,l:6}:v<=0.70?{n:5,c:5,m:5,l:5}:v<=0.85?{n:4,c:4,m:4,l:4}:
  {n:3,c:3,m:4,l:3};
...
function BTCThermalAI() {
  ...
  const checksBot = [
    ...
    { ok: G("utxoFlag")       === 1,        label: "UTXO Flag = 1 (fond <3%)"           },
  ...
  const checksTop = [
    ...
    { ok: G("utxoRatio")      >   0.80,     label: "UTXO Ratio > 80% (> ATH récent)"    },
  ...
  const rows = [
    ...
    },
    {
      ...
      sub_detail:<>
        <SubVal label="Ratio" val={f6(G("utxoRatio",1))} color={G("utxoRatio",1)<=0.03?"#ff4444":"#c9d1d9"}
          alert={<AlertR active={G("utxoFlag")===1} on="FLAG=1 🚨"/>}/>
        <SubVal label="SMA-365d [proxy]" val={f2(G("utxoSma365"))} color="#74c0fc"/>
        <SubVal label="Flag" val={G("utxoFlag")===1?"= 1 ⚡":"= 0"} color={G("utxoFlag")===1?"#ff4444":"#69db7c"}/>
      </>,
      sc:s.utxo, nowL:G("utxoFlag")===1?"FLAG\nACTIF":G("utxoRatio",1)<=0.10?"BAS\nCYCLE":"MILIEU",hz:"MT/LT"
    },
    {
      sec:null,
    ...
    <div style={{background:BG,color:"#c9d1d9",fontFamily:"Arial,sans-serif",minHeight:"100vh",padding:"20px 22px",
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:18,paddingBottom:16,borderBottom:`1px solid ${BORDER}`,flexWrap:"wrap",gap:12}}>
        <div>
          ...
          <div style={{fontSize:12,color:MUTED,marginTop:3}}>
            <code style={{color:"#74c0fc",background:"rgba(88,166,255,.1)",padding:"1px 6px",borderRadius:3,fontSize:11}}>btc_dashboard.json</code>
            &nbsp;·&nbsp;
            <code style={{color:"#69db7c",background:"rgba(46,204,113,.08)",padding:"1px 6px",borderRadius:3,fontSize:11}}>btc_pipeline.py</code>
        ...
        <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:8}}>
          ...
          <div style={{fontSize:11,color:MUTED}}>Thermal Score :&nbsp;
            <span style={{color:"#74c0fc",fontFamily:"monospace",fontWeight:700}}>{f2(G("thermalScore"))}/100</span></div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:18}}>
        ...
        <ScoreCard
          ...
          value={avgScore}
          sub={`Pipeline Composite: ${f2(G("thermalScore"))}/100 · Bottom ${scoreBot}/10 · Top ${scoreTop}/10`}
          color={avgColor}
          grad={avgGrad}
        />
      </div>
      ...
      <div style={{marginTop:18,paddingTop:14,borderTop:`1px solid ${BORDER}`,display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:8}}>
        <div style={{fontSize:11,color:MUTED,fontStyle:"italic"}}>
          Ce site est la propriété exclusive de Kizoka0x, qui en est l'auteur et l'éditeur.
        </div>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          <a href="https://t.me/Kizoka0x" target="_blank" rel="noopener noreferrer"
            ...
            <span style={{fontSize:16}}>✈</span> @Kizoka0x
          </a>
