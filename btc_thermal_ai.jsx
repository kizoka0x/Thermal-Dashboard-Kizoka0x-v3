...
//   mvrvPct < 50% ↔ ratio CQ 10-100 (normal bas)
//   mvrvPct < 80% ↔ ratio CQ 100-1k (bull moyen)
//   mvrvPct > 80% ↔ ratio CQ > 1k   (survalorisation)
const S_UTXO = v =>
  v===-1?{n:5,c:5,m:5,l:5}:   // indisponible → neutre
  v<=2?{n:1,c:2,m:4,l:9}:      // <2% = capitulation extrême (FLAG actif)
  v<=5?{n:1,c:3,m:5,l:9}:      // <5% = FLAG capitulation
...
function BTCThermalAI() {
  ...
  const avgScore = thermalDisplay;  // utilise le score thermique recalculé JSX (0-9)
  ...
  const checksBot = [
    ...
    { ok: G("utxoFlag")       === 1,        label: "UTXO Flag = 1 (mvrvPct ≤ 5% — capitulation CQ)"  },
  ...
  const rows = [
    ...
    {
      sec:null,
      name:"UTXO Block P/L Count Ratio [proxy mvrvPct]",
      sub_detail:<>
        <SubVal label="Proxy (mvrvPct %)" val={G("utxoRatio")===-1?"N/A":f2(G("utxoRatio"))+"%"} color={G("utxoRatio")===-1?"#4a5568":G("utxoRatio")<=5?"#ff4444":G("utxoRatio")<=15?"#ffa94d":"#c9d1d9"}
          alert={<AlertR active={G("utxoFlag")===1} on="FLAG=1 🚨 (mvrvPct<5%)"/>}/>
        <SubVal label="Zone" val={G("utxoRatio")===-1?"N/A":G("utxoRatio")<=2?"Capitulation extrême 🔴":G("utxoRatio")<=5?"FLAG Capitulation 🔴":G("utxoRatio")<=15?"Sous-valorisé 🟡":G("utxoRatio")<=55?"Normal":"Haussier"} color={G("utxoRatio")<5?"#ff4444":G("utxoRatio")<=15?"#ffa94d":G("utxoRatio")<=55?"#c9d1d9":"#69db7c"}/>
        <SubVal label="Flag" val={G("utxoFlag")===1?"= 1 ⚡ (mvrvPct ≤ 5%)":"= 0"} color={G("utxoFlag")===1?"#ff4444":"#69db7c"}/>
        <div style={{fontSize:8,color:"#4a5568",marginTop:3,fontStyle:"italic"}}>⚠ Proxy prix uniquement — ratio réel CQ indisponible (API payante)</div>
      </>,
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:18,paddingBottom:16,borderBottom:`1px solid ${BORDER}`,flexWrap:"wrap",gap:12}}>
        ...
        <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:8}}>
          ...
          <div style={{fontSize:11,color:MUTED}}>Thermal Score :&nbsp;
            <span style={{color:avgColor,fontFamily:"monospace",fontWeight:700}}>{avgScore}/9</span>
            <span style={{fontSize:9,color:"#4a5568",marginLeft:8}}>(recalculé JSX, 8 indicateurs)</span></div>
