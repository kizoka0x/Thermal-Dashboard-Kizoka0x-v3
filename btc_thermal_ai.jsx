...
//  - UTXO proxy = mvrvPct (0-100%) — ratio réel CQ indisponible sans API payante
// ══════════════════════════════════════════════════════════════════════════════
...
// UTXO Block P/L Count Ratio — PROXY via mvrvPct (0–100%)
// Le vrai ratio CQ (6–10000) est indisponible sans API payante.
// Notre proxy = mvrvPct (% des jours historiques où prix < prix actuel).
// Calibration sur données CQ réelles :
//   mvrvPct <  5% ↔ ratio CQ < 6   (capitulation FLAG)
//   mvrvPct < 15% ↔ ratio CQ < 10  (sous-valorisé)
//   mvrvPct < 50% ↔ ratio CQ 10-100 (normal bas cycle)
//   mvrvPct < 80% ↔ ratio CQ 100-1k (bull moyen)
//   mvrvPct > 80% ↔ ratio CQ > 1k   (survalorisation)
const S_UTXO = v =>
  v<=2?{n:1,c:2,m:4,l:9}:      // <2% = capitulation extrême
  v<=5?{n:1,c:3,m:5,l:9}:      // <5% = zone FLAG capitulation
  v<=15?{n:2,c:3,m:5,l:8}:     // <15% = sous-valorisé
  v<=35?{n:3,c:4,m:5,l:7}:     // <35% = bas de cycle
  v<=55?{n:5,c:5,m:5,l:6}:     // normal
  v<=75?{n:6,c:5,m:5,l:5}:     // haussier
  {n:7,c:6,m:5,l:4};            // survalorisation
...
// Sharpe Ratio annualisé — zone négative = low risk (achat LT), zone très positive = prise profit
// Logique : Sharpe très négatif = bas cycle (n bas, l haut), Sharpe très positif = top cycle (n haut, l bas)
// Pas de régression : chaque palier supérieur a un n >= palier précédent
const S_SHARPE = v =>
  v<=-1.5?{n:0,c:2,m:4,l:9}:v<=-0.8?{n:1,c:3,m:5,l:8}:v<=-0.3?{n:2,c:4,m:5,l:7}:
  v<=0.0?{n:4,c:5,m:5,l:6}:v<=0.5?{n:6,c:6,m:6,l:6}:v<=1.0?{n:7,c:6,m:6,l:5}:
  {n:8,c:7,m:5,l:4};
...
function BTCThermalAI() {
  ...
  const checksBot = [
    ...
    { ok: G("lthNupl")        <   0,        label: "LTH NUPL < 0 (holders LT en perte)"       },
    ...
    { ok: G("lthNupl")        <   0.20,     label: "LTH NUPL < 0.20 (bas cycle / espoir)"      },
  ];
  const scoreBot=checksBot.filter(c=>c.ok).length;
  const colorBot=scoreBot>=7?"#69db7c":scoreBot>=5?"#ffe066":scoreBot>=3?"#ffa94d":"#ff6b6b";
  const gradBot=scoreBot>=7?"linear-gradient(90deg,#27ae60,#2ecc71)":scoreBot>=5?"linear-gradient(90deg,#f39c12,#f1c40f)":scoreBot>=3?"linear-gradient(90deg,#e74c3c,#f39c12)":"linear-gradient(90deg,#c0392b,#e74c3c)";
  ...
  const checksTop = [
    ...
    { ok: G("mvrvPct")        >   80,       label: "MVRV / UTXO proxy > 80% (survalorisation)"    },
  ...
  const rows = [
    ...
    {
      sec:null,
      name:"UTXO Block P/L Count — proxy mvrvPct",
      sub_detail:<>
        <SubVal label="mvrvPct (proxy)" val={f2(G("utxoRatio"))+"%"} color={G("utxoRatio")<=5?"#ff4444":G("utxoRatio")<=15?"#ffa94d":G("utxoRatio")<=55?"#c9d1d9":"#69db7c"}
          alert={<AlertR active={G("utxoFlag")===1} on="FLAG 🚨 mvrvPct ≤ 5%"/>}/>
        <SubVal label="Zone" val={G("utxoRatio")<=2?"Capitulation extrême 🔴":G("utxoRatio")<=5?"FLAG Capitulation 🔴":G("utxoRatio")<=15?"Sous-valorisé 🟡":G("utxoRatio")<=55?"Normal":"Haussier"} color={G("utxoRatio")<=5?"#ff4444":G("utxoRatio")<=15?"#ffa94d":G("utxoRatio")<=55?"#c9d1d9":"#69db7c"}/>
        <SubVal label="Flag" val={G("utxoFlag")===1?"= 1 ⚡":"= 0"} color={G("utxoFlag")===1?"#ff4444":"#69db7c"}/>
        <div style={{fontSize:8,color:"#4a5568",marginTop:3,fontStyle:"italic"}}>Proxy prix — ratio réel CQ indisponible sans API</div>
      </>,
      sc:s.utxo, nowL:G("utxoFlag")===1?"FLAG\nACTIF":G("utxoRatio")<=15?"SOUS\nVALO":G("utxoRatio")<=55?"NORMAL":"HAUSSIER",hz:"MT/LT"
    },
    {
      sec:null,
  ...
  return (
    <div style={{background:BG,color:"#c9d1d9",fontFamily:"Arial,sans-serif",minHeight:"100vh",padding:"20px 22px",
      ...
      {/* ── ZONES PRIX — dynamiques selon données actuelles ────────────────── */}
      {(()=>{
        const btc   = G("btcPrice");
        const sma200= G("mayer_sma200");
        const sopr  = G("soprRatio",1);
        const mvrv  = G("mvrvPct");
        const lth   = G("lthNupl");

        // Zone 1 : Prix actuel — assessment dynamique
        const z1ok  = G("etf_30d_sum")<0||G("bullBear30d")<-0.10;
        const z1lbl = z1ok?"⚠ Résistance — signaux baissiers persistants":"→ Consolidation — surveiller déclencheurs";

        // Zone 2 : Support STH cost basis — approx. 90% du prix actuel
        const z2price= Math.round(btc * 0.92 / 1000) + "–" + Math.round(btc * 0.97 / 1000) + "K";
        const z2lbl  = lth < 0 ? "⬇ STH en perte — zone support probable" : "⬇ STH cost basis estimé";

        // Zone 3 : Cible scénario central — Mayer 0.80 × SMA200
        const z3lo   = Math.round(sma200 * 0.75 / 1000);
        const z3hi   = Math.round(sma200 * 0.85 / 1000);
        const z3     = `$${z3lo}–${z3hi}K`;
        const z3lbl  = sopr < 0.98 ? "🎯 Bottom zone historique (SOPR < 1)" : "🎯 Mayer 0.75–0.85 × SMA200";

        // Zone 4 : Capitulation extrême — Mayer 0.60 × SMA200
        const z4lo   = Math.round(sma200 * 0.55 / 1000);
        const z4hi   = Math.round(sma200 * 0.68 / 1000);
        const z4     = `$${z4lo}–${z4hi}K`;
        const z4lbl  = G("soprAlert")===1 && G("utxoFlag")===1 ? "⚡ SOPR Alert + UTXO Flag actifs!" : "⚡ Si SOPR Alert + UTXO Flag = 1";

        const zones=[
          {z:`$${Math.round(btc/1000).toFixed(0)}K (actuel)`, p:"Mayer "+G("mayerMultiple",1).toFixed(3)+" · MVRV "+f2(mvrv)+"%", l:z1lbl,       c:"#e74c3c",a:"#c0392b"},
          {z:z2price,                                          p:"STH NUPL "+fsign(G("sthNupl"),3)+" · zone tampon", l:z2lbl,                        c:"#ffa94d",a:"#f39c12"},
          {z:z3,                                               p:"SMA200 = $"+Math.round(sma200/1000)+"K × 0.75–0.85", l:z3lbl,                     c:"#ffe066",a:"#f1c40f"},
          {z:z4,                                               p:"Capitulation extrême (réf. 2022)", l:z4lbl,                                        c:"#da77f2",a:"#9b59b6"},
        ];
        return (
      ...
      <div style={{marginTop:16,display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
        ...
        <div style={{background:PANEL,border:`1px solid ${BORDER}`,borderRadius:10,padding:16,borderLeft:"3px solid #58a6ff"}}>
          ...
          <div style={{marginTop:10,paddingTop:8,borderTop:`1px solid ${BORDER}`,fontSize:12}}>
            ...
            <span style={{color:"#718096"}}>
              {scoreBot>=8?"⚡ Signal d'achat fort — confirmer price action":
               scoreBot>=6?"⏳ Convergence LT — attendre déclencheurs CT (SOPR + UTXO)":
               scoreBot>=4?"⏳ Signaux LT actifs — pression CT non résolue":
               scoreBot>=2?"❌ Prématuré — distribution active, attendre retournement":
               "❌ Aucune condition réunie — phase de distribution"}
            </span>
          </div>
          {/* Résumé dynamique des conditions manquantes */}
          <div style={{marginTop:6,fontSize:10,color:"#718096"}}>
            Manquants : {checksBot.filter(c=>!c.ok).map(c=>c.label).join(" · ")||"Tous validés ✓"}
          </div>
          <div style={{marginTop:6,fontSize:10,color:MUTED,fontStyle:"italic",borderTop:`1px solid rgba(255,255,255,.04)`,paddingTop:8}}>
            Signal fort = SOPR Alert=1 + UTXO Flag=1 + Mayer&lt;0.80 simultanés<br/>
            Réf. capitulation 2022 : SOPR≈0.54, SMA90≈0.65, Mayer≈0.67, LTH NUPL≈-0.49
          </div>
        </div>
        ...
        <div style={{background:PANEL,border:`1px solid ${BORDER}`,borderRadius:10,padding:16,borderLeft:"3px solid #e74c3c"}}>
          ...
          <div style={{marginTop:10,paddingTop:8,borderTop:`1px solid ${BORDER}`,fontSize:12}}>
            ...
            <span style={{color:"#718096"}}>
              {scoreTop>=8?"🔴 Signal de vente fort — distribution en cours":
               scoreTop>=6?"⚠ Signaux de sommet avancés — prudence":
               scoreTop>=4?"⚠ Signaux de sommet en formation — surveiller":
               "✅ Marché non en euphorie — bottom scenario prioritaire"}
            </span>
          </div>
          <div style={{marginTop:6,fontSize:10,color:"#718096"}}>
            Actifs : {checksTop.filter(c=>c.ok).map(c=>c.label).join(" · ")||"Aucun signal top"}
          </div>
          <div style={{marginTop:6,fontSize:10,color:MUTED,fontStyle:"italic",borderTop:`1px solid rgba(255,255,255,.04)`,paddingTop:8}}>
            Signal top = SOPR &gt; 6.9 + MVRV &gt; 85% + Mayer &gt; 2.4 + LTH NUPL &gt; 0.70<br/>
            Réf. tops 2021 : SOPR≈15.8, MVRV≈95%, Mayer≈3.5, LTH NUPL≈0.85
          </div>
        </div>
