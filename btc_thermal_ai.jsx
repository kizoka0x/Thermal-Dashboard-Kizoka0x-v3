...
//  - UTXO Flag = 1 quand ratio proxy P/L < 0.10 (fond extrême : quasi-tous UTXOs en perte)
// ══════════════════════════════════════════════════════════════════════════════
...
function BTCThermalAI() {
  ...
  const checksBot = [
    ...
    { ok: G("utxoFlag")       === 1,        label: "UTXO Flag = 1 (ratio proxy < 0.10 — capitulation)"      },
  ...
  const checksTop = [
    ...
    { ok: G("utxoRatio")      >   2.0,      label: "UTXO Ratio > 2.0 (bull fort — majorité en profit)"  },
  ...
  const rows = [
    ...
    },
    {
      ...
      sub_detail:<>
        <SubVal label="Ratio P/L proxy" val={f4(G("utxoRatio",1))} color={G("utxoRatio",1)<0.10?"#ff4444":G("utxoRatio",1)<0.30?"#ffa94d":"#c9d1d9"}
          alert={<AlertR active={G("utxoFlag")===1} on="FLAG=1 🚨"/>}/>
        <SubVal label="Flag seuil (<0.10)" val={G("utxoRatio",1)<0.10?"< 0.10 ⚠":"≥ 0.10 ✓"} color={G("utxoRatio",1)<0.10?"#ff4444":"#74c0fc"}/>
        <SubVal label="Flag" val={G("utxoFlag")===1?"= 1 ⚡":"= 0"} color={G("utxoFlag")===1?"#ff4444":"#69db7c"}/>
      </>,
      sc:s.utxo, nowL:G("utxoFlag")===1?"FLAG\nACTIF":G("utxoRatio",1)<0.30?"BAS\nCYCLE":"NEUTRE",hz:"MT/LT"
    },
    {
      sec:null,
