...
def compute_utxo_ratio(prices):
    """
    Proxy ratio UTXO P/L Block Count (CryptoQuant).
    = nb jours où prix < prix actuel (en profit)
    / nb jours où prix >= prix actuel (en perte)
    Nécessite une fenêtre 730j pour être représentatif.
    Valeur ~0.5-2 = marché normal, <0.10 = fond extrême (flag), >3 = bull fort.
    """
    profit = float((prices < prices.iloc[-1]).sum())
    loss   = float(max(1, (prices >= prices.iloc[-1]).sum()))
    return profit / loss
...
def run():
    ...
    # ── Données de base ──────────────────────────────────────────────────────
    prices    = get_btc_history(730)  # 2 ans pour proxy UTXO P/L fiable
    ...
    utxo_val  = utxo_real if utxo_real          else compute_utxo_ratio(prices)
    ...
    # Flag = 1 quand ratio proxy < 0.10 (fond extrême calibré sur données CQ)
    # ratio < 0.10 → quasi-tous les UTXOs en perte → capitulation historique
    utxo_flag = 1 if utxo_val < 0.10 else 0
    ...
    dashboard = {
        ...
        "utxoRatio":        round(utxo_val, 6),   # Ratio proxy P/L (profit_days/loss_days)
        "utxoSma365":       round(utxo_val, 4),         # Alias ratio (affiché dans le dashboard)
