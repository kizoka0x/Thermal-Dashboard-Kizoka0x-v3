...
def rbn_get(metric, limit=3, token=None):
    ...
    try:
        ...
        items = data.get("data") or data.get("timeseries") or (data if isinstance(data, list) else [])
        if items:
            return items
    except Exception as e:
    ...
    try:
        ...
        data = fetch(url, headers=heads)
        items = data.get("timeseries") or data.get("data") or (data if isinstance(data, list) else [])
        return items
    except Exception as e:
...
def get_rbn_onchain(token):
    ...
    Total : ~9 requêtes (bien dans le quota Tier 0 de 55k DP/semaine).
    """
    if not token:
    ...
    # ── SOPR LTH/STH réels ───────────────────────────────────────────────────
    # Récupère 90 jours de LTH-SOPR pour le SMA90
    lth_sopr_series = rbn_get("sopr_lth", limit=90, token=token)
    sth_sopr_series = rbn_get("sopr_sth", limit=3,  token=token)
...
def get_bg_complement(token, needed):
    ...
    if "stable30dChg" in needed:
        d = bg_get("stablecoin-supply", last=35, token=token)
        if d and len(d) >= 2:
            def _b(v):
            ...
            usdt_prev = _b(d[0].get("usdt"))
            usdc_prev = _b(d[0].get("usdc"))
            result["stableUsdtB"]  = round(usdt_now, 3)
            result["stableUsdcB"]  = round(usdc_now, 3)
            result["stable30dChg"] = round((usdt_now + usdc_now - usdt_prev - usdc_prev) * 1e9, 2)
            print(f"  BG Stable → USDT={usdt_now:.1f}B  USDC={usdc_now:.1f}B  30d_chg={result['stable30dChg']/1e6:.0f}M$")
...
def get_binance_derivatives():
    ...
    try:
        ...
        cvd_vals = [(float(k[9]) - (float(k[5]) - float(k[9]))) * float(k[4]) for k in klines]
    ...
    try:
        ...
        print(f"  Binance NTV 25h → {ntv/1e6:.1f}M$")
    except Exception as e:
        print(f"  ⚠ NTV: {e}")
...
def get_btc_history(days=365):
    # limit=365 explicite — sans ce paramètre, Coinbase retourne 300 bougies par défaut
    # (source : doc Coinbase Exchange API /products/{id}/candles, max=300 sans paramètre)
    # La SMA-200 et le bull/bear 365j nécessitent au minimum 365 points.
    # Coinbase accepte jusqu'à 300 bougies par requête → on fait 2 appels pour couvrir 400j.
    import time as _time
    closes = []
    # Appel 1 : 300 derniers jours
    try:
        d1     = fetch("https://api.exchange.coinbase.com/products/BTC-USD/candles?granularity=86400&limit=300")
        c1     = [float(c[4]) for c in d1]
        c1.reverse()
        closes = c1
    except Exception as e:
        print(f"  ⚠ Coinbase candles lot1: {e}")

    # Appel 2 : 100 jours supplémentaires (end = start du lot 1)
    # → total ~400 jours pour absorber les week-ends sans données
    if len(closes) >= 10:
        try:
            _time.sleep(0.5)
            # end = timestamp du plus ancien point déjà récupéré
            end_ts = int(__import__('datetime').datetime.utcnow().timestamp()) - len(closes) * 86400
            url2   = (f"https://api.exchange.coinbase.com/products/BTC-USD/candles"
                      f"?granularity=86400&limit=100&end={end_ts}")
            d2     = fetch(url2)
...
def assemble_usdt(data, prices):
    """Stablecoin supply réelle (BG) → variation USD."""
    chg = data.get("stable30dChg")
    if chg is not None:
        return {
            "usdt_daily_mc":     round(float(chg) / 30, 2),
            "usdt_sma30":        round(float(chg), 2),
            "usdt_60d_change":   round(float(chg) * 2, 2),
            "usdt_60d_sma30":    round(float(chg) / 30, 2),
    ...
    # Fallback CoinGecko
    try:
        cg    = fetch("https://api.coingecko.com/api/v3/coins/tether/market_chart?vs_currency=usd&days=32")
        pser  = pd.Series([p[1] for p in cg["prices"]])
        s30   = float((pser.iloc[-1] - pser.rolling(30).mean().iloc[-1]) * 1e9)
        daily = float(pser.iloc[-1] - pser.iloc[-2]) * 1e9
        print(f"  ⚡ Stablecoin CoinGecko")
        return {"usdt_daily_mc": round(daily, 2), "usdt_sma30": round(s30, 2),
                "usdt_60d_change": 0.0, "usdt_60d_sma30": 0.0,
...
def compute_thermal_score(d):
    """Score composite 0–9 via 9 indicateurs pondérés (inchangé)."""
    def _s(v, lo, hi):
        if v is None: return 4.5
        return float(max(0, min(9, (v - lo) / (hi - lo) * 9)))
    scores = [
